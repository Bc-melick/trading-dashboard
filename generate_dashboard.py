...
def fred_series(series_id, api_key, limit=2):
    """Fetch the latest N observations from FRED."""
    if not api_key:
        return None
...
fed_funds   = fred_series('FEDFUNDS',  FRED_API_KEY)   # Fed Funds Rate
cpi_yoy     = fred_series('CPIAUCSL',  FRED_API_KEY)   # CPI (need to calc YoY)
unemployment= fred_series('UNRATE',    FRED_API_KEY)   # Unemployment Rate
ten_yr      = fred_series('GS10',      FRED_API_KEY)   # 10-Year Treasury
...
def trend_arrow(vals):
    if vals and len(vals) >= 2:
        try:
            return '▲' if float(vals[0]) > float(vals[1]) else '▼'
        except:
            return ''
    return ''
...
fig1 = go.Figure()
fig1.add_trace(go.Scatter(x=blended_price.index, y=blended_price, mode='lines',
...
for ema, lbl, col in [(ema_20,'20 EMA','#f87171'),(ema_50,'50 EMA','#4ade80'),
    ...
    fig1.add_trace(go.Scatter(x=ema.index, y=ema, mode='lines', name=lbl,
        line=dict(width=1, dash='dot', color=col), opacity=0.7, hoverinfo='skip'))
if buy_signals:
    fig1.add_trace(go.Scatter(x=[s[0] for s in buy_signals], y=[s[1] for s in buy_signals],
...
if reduction_signals:
    fig1.add_trace(go.Scatter(x=[s[0] for s in reduction_signals], y=[s[1] for s in reduction_signals],
        mode='markers', name='Reduce', marker=dict(color='#f87171', size=10, symbol='triangle-down'),
        hovertemplate='REDUCE<br>%{x|%Y-%m-%d}<br>%{y:.2f}<extra></extra>'))

buttons1 = [dict(label=lbl, method='relayout',
    args=[{'xaxis.range': [pd.Timestamp(ts), pd.Timestamp(te)],
           'yaxis.range': yr(ts, te, blended_price)}])
...
fig1.update_layout(
    ...
    yaxis=dict(title='Blended Price', gridcolor='#334155'),
...
fig2.add_trace(go.Scatter(x=bt_results.index, y=bt_results['Portfolio_Value'],
    mode='lines', name='Strategy', line=dict(width=2, color='#60a5fa'),
    hovertemplate='%{x|%Y-%m-%d}<br>$%{y:,.0f}<extra></extra>'), row=1, col=1)
fig2.add_trace(go.Scatter(x=bt_results.index, y=bt_results['Benchmark_Value'],
    mode='lines', name='SPY B&H', line=dict(width=2, color='#fb923c', dash='dash'),
    hovertemplate='%{x|%Y-%m-%d}<br>$%{y:,.0f}<extra></extra>'), row=1, col=1)
fig2.add_trace(go.Scatter(x=bt_results.index, y=bt_results['Exposure_Pct'],
...
buttons2 = [dict(label=lbl, method='relayout',
    args=[{'xaxis.range': [pd.Timestamp(ts), pd.Timestamp(te)],
           'yaxis.range': yr(ts, te, bt_results['Portfolio_Value'])}])
    for lbl, ts, te in timeframes]
fig2.update_layout(
    paper_bgcolor='#0f172a', plot_bgcolor='#1e293b', font=dict(color='#e2e8f0'),
...
fig2.update_yaxes(title_text='Portfolio Value ($)', gridcolor='#334155', row=1, col=1)
...
ann_rows = [[r['Year'], fmt_pct(r['Strategy']), fmt_pct(r['SPY']), fmt_pct(r['Alpha'])]
...
geo_html   = news_list(geo_news,   'Add NEWS_API_KEY secret to enable news.')
macro_html = news_list(macro_news, 'Add NEWS_API_KEY secret to enable news.')
